...
main(1,training_matrix,test_matrix);
 ...
 function main(cutoff, training_matrix,test_matrix)
       ...
       % seq=train_energy_table(min_idx,1);
        ...
        for i=1:400:(size_table-97)
           ...
           %  writecell(u_list,"partialEE_original/energies_"+ int2str(i) + ".xls",'Sheet',1,'Range','A1')
             writecell(e_diff_accepted,"fullEE/single/energies_"+ int2str(i) + ".xlsx",'Sheet',1,'Range','B1');
             writecell(e_diff_rejected,"fullEE/single/energies_"+ int2str(i) + ".xlsx",'Sheet',1,'Range','C1');

           % xlswrite("partialEE_original/post_sampling_mcmc_partial_energy_t001_" + int2str(i) + ".csv",final_array,'Sheet1'); 
            writetable(final_array,("fullEE/single/post_sampling_mcmc_partial_energy_t001_" + int2str(i) + ".csv")); 
        end
  ...
  energy_table=table(file,all_energies);
    ...
    function new_seq_initial=mutate_seqs(seq);
            perm=randi(4);
            loc=randi(30);
            seq(loc)=perm;
            new_seq_initial=seq;
    ...
    function [new_seq_initial,mutated_seq]=check_seq(new_seq_initial,seq)
             if sum(new_seq_initial-seq)~=0
                mutated_seq=new_seq_initial;
             else;
                new_seq_initial=mutate_seqs(seq);
             end 
     end 
%     
%     %eliminates seq repetition 
%     function new_mutated_seq=mutate_one_base(old_seq);
%         new_seq=old_seq;
    ...
    function [new_seq_list,energy_list,e_diff_accepted,e_diff_rejected]=mcmc(seq,JJ,hi) %,seq_letters);
        ...
        % list mutated seq here 
           for i=1:n_iterations;
             new_seq_initial=mutate_seqs(seq);
             mutated_seq=check_seq(new_seq_initial,seq);
           % perm_mutated_seq=mutate_any_base(seq)
             ...
             if u <= min(1,e_diff);
                 ...
                 energy_list(counter,1)=compute_energy_seq(JJ,hi,seq);
