...
%         
        for i=1:400:(size_table-97)
           ...
           %  writecell(u_list,"partialEE_original/energies_"+ int2str(i) + ".xls",'Sheet',1,'Range','A1')
            writecell(e_diff_accepted,"fullE_original/energies_"+ int2str(i) + ".xlsx",'Sheet',1,'Range','A1');
            writecell(e_diff_rejected,"fullE_original/energies_"+ int2str(i) + ".xlsx",'Sheet',1,'Range','B1');

            %xlswrite(("partialE/post_sampling_mcmc_partial_energy" + int2str(i) + ".csv"),final_array); 
            writetable(final_array,("fullE_original/post_sampling_full_energy_t001_" + int2str(i) + ".csv")); 
        end
...
%         percent_int=100*size(int,1)/size_to_eval    
  ...
  energy_table=[file,all_energies];
    ...
    function y=prob_density(seq,beta,JJ,hi,seq_letters)
        y=exp((-beta*compute_energy_seq(JJ,hi,seq,seq_letters)));  % need to work on this here 
    end 
    ...
    function [new_seq_list,energy_list,e_diff_accepted,e_diff_rejected]=mcmc(seq,JJ,hi,seq_letters);
     ...
     % rejected sequences 
%         rejected_energy_list=zeros(n_iterations,1);
%         rejected_seq_list=zeros(n_iterations,30);
%         reject_counter= 1;
...
%           counteru=1;
%           u_list=zeros(n_iterations,1);

        for i=1:n_iterations;
             mutated_seq=mutate_seqs(seq);
             e_diff=prob_density(mutated_seq,beta,JJ,hi,seq_letters)/prob_density(seq,beta,JJ,hi,seq_letters);
             seq=mutated_seq;
             u=rand(1);
             if u <= min(1,e_diff);
                 energy=compute_energy_seq(JJ,hi,seq,seq_letters); %% full e
                 new_seq_list(counter,:)=seq;
                 energy_list(counter,1)=compute_energy_seq(JJ,hi,seq,seq_letters);
                 e_diff_accepted(counter,1)=e_diff;
                 counter=counter+1;
             else
%                  reject_energy=compute_energy_motif_seq(JJ,hi,seq,seq_letters);
%                  rejected_seq_list(reject_counter,:)=seq;
%                  rejected_energy_list(reject_counter,1)=compute_energy_motif_seq(JJ,hi,seq,seq_letters);  
                 e_diff_rejected(reject_counter,1)=e_diff;
