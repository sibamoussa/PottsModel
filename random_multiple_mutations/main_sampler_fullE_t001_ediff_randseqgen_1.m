       ...
       % seq=train_energy_table(min_idx,1);
        ...
        for i=1:400:(size_table-97)
           ...
           %  writecell(u_list,"partialEE_original/energies_"+ int2str(i) + ".xls",'Sheet',1,'Range','A1')
             writecell(e_diff_accepted,"fullEE/multiple/energies_"+ int2str(i) + ".xlsx",'Sheet',1,'Range','B1');
             writecell(e_diff_rejected,"fullEE/multiple/energies_"+ int2str(i) + ".xlsx",'Sheet',1,'Range','C1');

           % xlswrite("partialEE_original/post_sampling_mcmc_partial_energy_t001_" + int2str(i) + ".csv",final_array,'Sheet1'); 
            writetable(final_array,("fullEE/multiple/post_sampling_mcmc_partial_energy_t001_" + int2str(i) + ".csv")); 
        end
